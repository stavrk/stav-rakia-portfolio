    ...
    <section className="py-20 bg-secondary/50">
      <div className="container">
        ...
        >
          ...
          <p className="text-content text-lg max-w-2xl mx-auto">A strategic approach to creating intuitive and engaging user experiences</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {processSteps.map((step, index) => (
            <motion.div 
              key={index}
              className="flex flex-col items-center text-center p-6 rounded-xl bg-background border border-border/40"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-100px" }}
            ...
            >
              ...
              <p className="text-content">{step.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
