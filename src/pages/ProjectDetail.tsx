...
const projectsData = [{
  ...
  designProcessAndAesthetics: "Visual Design\nStudit's visual language balances academic credibility with engaging interactivity. The color palette combines warm neutrals with muted blues and strategic yellow accents, creating focus points while maintaining a calm environment for concentrated study.\n\nTypography\nWe selected Assistant as our primary font for its excellent readability and Hebrew character support, complemented by Indie Flower for accent elements. The text hierarchy relies on weight variations rather than size differences, with a minimum 16px body text to support extended reading sessions.\n\nLayout Structure\nThe interface uses generous white space and a consistent grid to organize complex information. Content appears in cards with subtle shadows and 4px rounded corners, while interactive elements feature pronounced shadows and state changes to signal functionality.\n\nUser Flow\nThe interface guides users through a carefully sequenced journey: homepage → exam selection → location selection → diagnostic assessment → personalized scheduling. Animated transitions between stages provide both functional feedback and moments of delight in what could otherwise be a stressful process.",
